...
function get_skin_dir($skin, $skin_path=G5_PATH.'/thema/'.THEMA.'/skin')
{
    global $g5;
...
// 권한 검사
function auth_check($auth, $attr, $return=false, $json=false)
{
    global $is_admin;
...
# 관리자 개인정보 접근이력 기록
function nx_privacy_log($task, $tbname = null, $tbpk = null) {
    global $member;

    if ($task == '' || !$member['mb_id']) return;

    $file = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);

    $sql = "Insert into PRIVACY_LOG ("
        ."mb_id"
        .", PL_FILE_NAME"
        .", PL_TASK"
        .", PL_TBNAME"
...
if (!$member['mb_id'])
{
    ...
    else if (strpos($_SERVER['REQUEST_URI'], "/udong-mng/") !== false) {
        goto_url(G5_URL.'/udong-mng/login.php');
    }
    else if (strpos($_SERVER['REQUEST_URI'], "/udong/adm/") !== false) {
        goto_url(G5_URL.'/udong/adm/login.php');
    }
    else if (strpos($_SERVER['REQUEST_URI'], "/hr/") !== false) {
        goto_url(G5_URL.'/hr/login.php');
    }
    else {
        goto_url(G5_ADMIN_URL.'/login.php?url=' . urlencode(G5_ADMIN_URL));
    }
}
else if ($is_admin != 'super')
{
    ...
    if (!$i)
    {
        # 우리동네 학습공간 시설 관리자, 공모업체 관리자가 아니면서 관리 권한이 없는 경우
        if ($member['mb_level'] != 4 && $member['mb_level'] != 5 && $member['mb_level'] != 6 && $member['mb_level'] != 7 && $member['mb_level'] != 3) {
            alert('최고관리자 또는 관리권한이 있는 회원만 접근 가능합니다.', G5_URL);
        }
        else {
            # 4레벨인 공모업체 관리자이면서 udong-mng로 접근하지 않았을때
            if ($member['mb_level'] == 4 && strpos($_SERVER['REQUEST_URI'], "/udong-mng/") === false) {
                goto_url(G5_URL);
            }
            # 5레벨인 공모업체 관리자이면서 project로 접근하지 않았을때
            if ($member['mb_level'] == 5 && strpos($_SERVER['REQUEST_URI'], "/project/") === false) {
                goto_url(G5_URL);
            }
            
            # 6레벨인 교육강사 관리자이면서 hr로 접근하지 않았을때
            if ($member['mb_level'] == 6 && strpos($_SERVER['REQUEST_URI'], "/hr/") === false) {
                goto_url(G5_URL);
            }
            
            # 7레벨인 교육강사 관리자이면서 hr로 접근하지 않았을때
            if ($member['mb_level'] == 7 && strpos($_SERVER['REQUEST_URI'], "/hr/") === false) {
                goto_url(G5_URL);
            }
            # 4레벨인 공모업체 관리자이면서 udong-mng로 접근하지 않았을때
            if ($member['mb_level'] == 3 && strpos($_SERVER['REQUEST_URI'], "/udong/adm/") === false) {
                goto_url(G5_URL);
            }
        }
    }
