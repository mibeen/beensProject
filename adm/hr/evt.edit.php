<?php
	...
	$sql = "Select EM.*"
		...
		. " 	Left Join NX_EVENT_HR As EP On EP.EP_IDX = EM.EP_IDX"
	...
	$g5[title] = ($sub_menu == '985100') ? "기관 강사수정" : "강사관리";
		...
		<tr>
			...
			<td>
				<div class="radio1_wrap">
					...
					<input type="radio" id="EM_JOIN_TYPE2" name="EM_JOIN_TYPE" class="radio1" value="2" <?php if ($db_em['EM_JOIN_TYPE'] != '1' && $db_em['EM_JOIN_TYPE'] != '3') echo('checked');?> /><label for="EM_JOIN_TYPE2"><span class="radbox"><span></span></span><span class="txt">승인</span></label>
					<input type="radio" id="EM_JOIN_TYPE3" name="EM_JOIN_TYPE" class="radio1" value="3" <?php if ($db_em['EM_JOIN_TYPE'] == '3') echo('checked');?> /><label for="EM_JOIN_TYPE3"><span class="radbox"><span></span></span><span class="txt">없음</span></label>
				</div>
			</td>
		</tr>
...
var onclPreview = function() {
	...
	f.action = '<?php echo(G5_URL)?>/hr/evt.read.preview.php';
	f.method = 'post';
	f.target = '_blank';
	f.submit();
}
