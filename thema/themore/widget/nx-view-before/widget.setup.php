...
<div class="tbl_head01 tbl_wrap">
	...
	<tr>
		<td align="center">제목 표시 여부</td>
		<td>
			<select name="wset[title]">
				<option value="0"<?php echo get_selected('0', $wset['title']);?>>표시 안함</option>
				<option value="1"<?php echo get_selected('1', $wset['title']);?>>표시</option>
			</select>
		</td>
	</tr><!--
	<tr class="hidden">
	...
	<tr>
		<td align="center">왼쪽방향</td>
		<td>
			<?php echo help('체크하시면 화살표 및 텍스트의 방향이 왼쪽이 됩니다. 모바일일 경우 위쪽 방향으로 나오게 됩니다.');?><br>
			<input type="checkbox" name="wset[view]" value="left" class="frm_radio" <?php echo ($wset['view'] == 'left') ? 'checked' : ''; ?>>
		</td>
	</tr>
	<tr>
		...
		<td>
			<select name="wset[color]">
				<option value="red" <?php echo get_selected('red', $wset['color']); ?>>red</option>
				<option value="yellow" <?php echo get_selected('yellow', $wset['color']); ?>>yellow</option>
				<option value="green" <?php echo get_selected('green', $wset['color']); ?>>green</option>
				<option value="blue" <?php echo get_selected('blue', $wset['color']); ?>>blue</option>
			</select>
		</td>
	</tr>
<!--
...
-->
	...
	<tr>
		<td align="center">추출할 글의 id(wr_id)</td>
		<td>
			<?php echo help('상단의 추출 보드 안에 있는 게시물들 중, 추출하려는 글의 wr_id를 입력해야 합니다. ');?>
			<input type="text" name="wset[where]" value="<?php echo $wset['where']; ?>" size="60" class="frm_input">
			&nbsp;
		</td>
	</tr>
	<!--
