...
for ($i=$chk_count-1; $i>=0; $i--)
{
    ...
    while ($row = sql_fetch_array($result))
    {
		...
		// 신고글
		sql_query(" delete from {$g5['apms_shingo']} where bo_table = '$bo_table' and wr_id = '{$row['wr_id']}' ", true);
	}

    // 게시글 삭제
    sql_query(" delete from $write_table where wr_parent = '{$write['wr_id']}' ", true);

    // 최근게시물 삭제
    $db_new = sql_fetch("Select Count(*) as cnt From {$g5['board_new_table']} where bo_table = '$bo_table' and wr_parent = '{$write['wr_id']}'");
    $new_cnt = $db_new['cnt'];
	...
	// 내글반응 삭제
	sql_query(" delete from {$g5['apms_response']} where bo_table = '$bo_table' and wr_id = '{$write['wr_id']}' ", true);

	// 태그로그 삭제
	sql_query(" delete from {$g5['apms_tag_log']} where bo_table = '$bo_table' and wr_id = '{$write['wr_id']}' ", true);

	// 이벤트 삭제
	sql_query(" delete from {$g5['apms_event']} where bo_table = '$bo_table' and wr_id = '{$write['wr_id']}' ", true);

	// 설문 삭제
	sql_query(" delete from {$g5['apms_poll']} where bo_table = '$bo_table' and wr_id = '{$write['wr_id']}' ", true);

	// 플레이목록
	sql_query(" delete from {$g5['apms_playlist']} where bo_table = '$bo_table' and wr_id = '{$write['wr_id']}' ", true);
