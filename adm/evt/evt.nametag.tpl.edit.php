<?php
	...
	*/
	...
	# set : variables
	$ENT_IDX = (isset($_POST['ENT_IDX']) && $_POST['ENT_IDX'] != '') ? $_POST['ENT_IDX'] : $_GET['ENT_IDX'];


	# re-define
	$ENT_IDX = (CHK_NUMBER($ENT_IDX) > 0) ? (int)$ENT_IDX : (int)0;


	# chk : rfv.
	if ($ENT_IDX <= 0) {
		header("location:evt.nametag.tpl.list.php?".$epTail);
		exit();
	}


	# get data
	$db_ent = sql_fetch("Select * From NX_EVENT_NAMETAG_TEMPLATE Where ENT_DDATE is null And EM_IDX is null And ENT_IDX = '" . mres($ENT_IDX) . "' Limit 1");
...
<input type="hidden" id="ENT_IDX" name="ENT_IDX" value="<?php echo((int)$ENT_IDX)?>" />
...
<table border="0" cellpadding="0" cellspacing="0" class="nx-t-read1">
	...
	<tbody>
		<tr>
			<th>템플릿명</th>
			<td>
				<input type="text" id="ENT_TITLE" name="ENT_TITLE" maxlength="100" value="<?php echo(F_hsc($db_ent['ENT_TITLE']))?>" class="nx-ips1 wl" />
			</td>
		</tr>
		<tr>
			<th>사이즈</th>
		...
		<tr>
			...
			<td>
				...
				$_files = get_file('NX_EVENT_NAMETAG_TEMPLATE', $ENT_IDX);
...
<div class="ofH mt10">
	...
	<div class="fR">
		<a href="javascript:void(0)" onclick="tpl.copy();" class="nx-btn-b3">복제</a>
