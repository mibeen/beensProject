<?php
    ...
    $rowCnt = ceil($rowCnt / (60*60*24*30*12));
    ###################################################################################################
    for ($i=0; $i<$rowCnt; $i++){
        ${"todayYear_".$i} = date("Y", strtotime("-".$i." years"));
        $array_todayYear[$i] = ${"todayYear_".$i};
        ...
        while($row = sql_fetch_array(${"result_joinuser_year_".$i})){
        ...
        while($row = sql_fetch_array(${"result_loginuser_year_".$i})){
        ...
        while($row = sql_fetch_array(${"result_conuser_year_".$i})){
            $array_conuser[$i] = $row['cnt'];
        } 
    }
    ###################################################################################################
	...
	*/
	...
	header("Content-Disposition: attachment; fileName=".iconv('utf-8', 'euc-kr', '년별 방문_접속_가입자 통계')."_" . date("ymd") . ".csv");
