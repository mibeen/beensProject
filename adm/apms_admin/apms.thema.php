...
if($mode == 'thema') {
	...
	if($_POST['as_intro_skin'] || $_POST['as_mobile_intro_skin']) {
		if($_POST['as_intro_type'] == "2") {
			$as_intro_set = $_POST['as_intro_type'].'|'.strtotime($_POST['as_intro_sday'].' '.$_POST['as_intro_shour'].':00:00').'|'.strtotime($_POST['as_intro_eday'].' '.$_POST['as_intro_ehour'].':00:00');
		} else {
	...
	$sql2 = " UPDATE g5_apms_data SET data_1 = '{$_POST['as_thema']}' WHERE type=11";
	sql_query($sql2, true);
...
$frm_submit .= '<a href="'.G5_ADMIN_URL.'/apms_admin/apms.exp.default.php" class="btn_frmline win_memo" style="display:none;">포인트 정리</a>'.PHP_EOL;
$frm_submit .= '</div>';
...
<div class="tbl_head01 tbl_wrap">
	...
	<?php 
		...
		list($as_intro_type, $as_intro_stime, $as_intro_etime) = explode("|", $config['as_intro']);
	?>
	<td align="center">
	...
	<td align="center">
		<input type="text" id="as_intro_sday"  name="as_intro_sday" value="<?php echo ($as_intro_stime) ? date("Y-m-d", $as_intro_stime) : '';?>" class="frm_input" size="10" maxlength="10"> 일 <br>
		<select id="as_intro_shour" name="as_intro_shour">
		<?php
			$as_intro_shour = ($as_intro_stime) ? date("H", $as_intro_stime) : '';
			for($i=0; $i < count($harr); $i++) {
				$selected = ($harr[$i] == $as_intro_shour) ? ' selected' : '';
				echo '<option value="'.$harr[$i].'"'.$selected.'>'.$harr[$i].'</option>'.PHP_EOL;
			}
		?>
		</select>
		시 부터 <br>
	    <input type="text" id="as_intro_eday"  name="as_intro_eday" value="<?php echo ($as_intro_etime) ? date("Y-m-d", $as_intro_etime) : '';?>" class="frm_input" size="10" maxlength="10"> 일 <br>
		<select id="as_intro_ehour" name="as_intro_ehour">
		<?php
			$as_intro_ehour = ($as_intro_etime) ? date("H", $as_intro_etime) : '';
			for($i=0; $i < count($harr); $i++) {
				$selected = ($harr[$i] == $as_intro_ehour) ? ' selected' : '';
				echo '<option value="'.$harr[$i].'"'.$selected.'>'.$harr[$i].'</option>'.PHP_EOL;
			}
		?>
		</select>
...
<div class="tbl_head01 tbl_wrap">
	...
	<td>경험치룰</td>
	<td>
		<label style="display:none;"><input type="checkbox" name="exp_point" value="1"<?php if($xp['exp_point']) echo ' checked';?>> 현재 회원 포인트(<?php echo AS_MP;?>) - 설정시 다른 룰은 무시되며, 포인트 사용에 따라 경험치와 레벨 등락 발생</label>
	...
	<td>
		...
		<a class="btn_frmline win_memo" href="<?php echo G5_ADMIN_URL; ?>/apms_admin/apms.exp.default.php" style="display:none;">포인트 및 경험치 정리</a>
	...
	<tr>
	<td><p style="display:none;">포인트</p></td>
	<td><p style="display:none;"><input type="text" name="as_mp" size="15" value="<?php echo $config['as_mp'];?>" class="frm_input"> 표현코드 : &lt;?php echo AS_MP;?></p></td>
	</tr>
	</tbody>
	</table>
</div>
...
<div class="local_desc01 local_desc">
	<p><b>● 기능 설정</b> - SEO, <span style="display:none;">럭키포인트,</span> API Key 등 설정</p>
</div>
...
<div class="tbl_head01 tbl_wrap">
	...
	<tr style="display:none;">
	<td><b>적립제한</b></td>
	<td><p style="display:none;">댓글점수</p></td>
...
<script>
$(function(){
	$("#as_intro_sday, #as_intro_eday, #as_shop_intro_day").datepicker({ changeMonth: true, changeYear: true, dateFormat: "yy-mm-dd", showButtonPanel: true });
});
</script>