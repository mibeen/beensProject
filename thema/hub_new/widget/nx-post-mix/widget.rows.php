...
//글쓴이
$is_name =  (isset($wset['wrname']) && $wset['wrname'] == "1") ? true : false;
	...
	?>
		<div class="media">
			<?php if($img[$i]['img']['src']) { // 있으면 출력 ?>
				<div class="bak_pull-<?php echo $is_right;?> post-image">
					<a href="<?php echo $img[$i]['href'];?>" class="ellipsis"<?php echo $is_modal_js;?><?php echo $target;?>>
						<div class="img-wrap">
							...
							<div class="img-item">
								<img src="<?php echo $img[$i]['img']['src'];?>" alt="<?php echo $img[$i]['img']['alt'];?>" width="<?php echo $wset['thumb_w'] ?>">
							</div>
						</div>
					</a>
				</div>
			...
			<div class="media-body">
				<div class="post-content">
					<div class="post-subject">
						<a href="<?php echo $img[$i]['href'];?>"<?php echo $is_modal_js;?><?php echo $target;?>>
							<p class="nx_post_title">
								<?php #echo $wr_icon;?>
								<?php echo cut_str($img[$i]['subject'],20);?>
							</p>
							<p class="nx_post_date">
								<?php #echo ($is_dtxt) ? apms_datetime($list[$i]['date'], $is_dtype) : date($is_dtype, $list[$i]['date']); ?>
								<?php echo date($is_dtype, strtotime($img[$i]['wr_datetime'])); ?>
							</p>
							<?php if($is_cont) { ?>
								<div class="post-text">
					...
					<div class="post-text post-ko txt-short ellipsis font-12" style="display: none;">
						<?php if($is_name)echo $img[$i]['name'];?>
	...
	<li class="ellipsis">
		<a href="<?php echo $list[$i]['href'];?>"<?php echo $is_modal_js;?><?php echo $target;?>>
			...
			<?php if($is_date || $list[$i]['comment']) { ?> 
				<span class="nx-post-date gray font-12">
