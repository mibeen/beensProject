...
function apms_photo_url($mb_id='') {
	...
	if (is_file($photo_file)){
		$mdate = date("YmdHis", filemtime($photo_file));
	}

	return $photo_url . '?ver=' . $mdate;
}
...
function apms_script($name){
	...
	if($name == 'code') {
		...
		return ' onclick="view_modal(this.href, this.getAttribute(\'data-modal-title\')); return false;"';
	}
	else if($name == 'modal_pop') {
		if(!defined('APMS_MODAL')) {
			define('APMS_MODAL', true);
			include_once(G5_LIB_PATH.'/apms.modal.lib.php');
		}
		
		# fixed : iOS 일 경우는 (iPhone,iPad) + Android 새창으로
		if (strpos($_SERVER['HTTP_USER_AGENT'], 'iPhone') !== false || strpos($_SERVER['HTTP_USER_AGENT'], 'iPad') !== false || strpos($_SERVER['HTTP_USER_AGENT'], 'Android') !== false) {
...
function apms_chk_auto_menu($s='', $mobile='', $type='') {
	...
	//보드
	$n = 0;
	$result = sql_query("select bo_table, bo_subject, bo_mobile_subject, bo_use_category, bo_category_list, bo_count_write, bo_count_comment, as_order, as_menu, as_line, as_sp, nx_depth2_yn, $sql_select from {$g5['board_table']} where as_show = '1' and bo_device <> '$device' $gr_in ", false);
	for ($i=0; $row=sql_fetch_array($result); $i++) {
	...
	//문서
	$result = sql_query("select id, html_id, bo_subject, bo_mobile_subject, as_file, as_html, as_order, as_line, as_menu, as_sp, nx_depth2_yn, $sql_select from {$g5['apms_page']} where gr_id <> '' and as_show = '1' and as_html <> '3' and bo_subject <> '' and html_id <> '' and bo_device <> '$device' $gr_in ", false);
	for ($i=0; $row=sql_fetch_array($result); $i++) {
		$pg[$row['gr_id']][$n] = $row;
		$n++;
	}
	...
	for ($i=0; $i < $gr_cnt; $i++) {
		...
		for ($k=0; $k < $bo_cnt; $k++) {
			...
			$list[$z]['sub'][$k]['depth2'] = $bo[$k]['nx_depth2_yn']; //타이틀메뉴
	...
	for($i=0;$i<count($imgs[1]);$i++) {
		...
		if(is_file($destfile) && strpos($destfile, G5_DATA_DIR.'/'.G5_EDITOR_DIR)) {
			if($mode == 'copy') { //다른이름으로 복사
				...
				//@unlink($destfile);
				$fp = @fopen(G5_DATA_PATH.'/log/unlink_log.txt', 'ab');
				if ($fp) {
				    @fwrite($fp, print_r("apm.lib : " . date("Y-m-d H:i:s",time()) . " : ".$destfile, true ) ."\n");
				    @fclose($fp);
				}
			}
		}
...
// 위젯타이틀링크
function apms_widget_config($wid, $opt='', $mopt='', $addon='', $mobile='N'){
	global $g5;
...
function apms_widget($wname, $wid='', $opt='', $mopt='', $wdir='', $addon=''){
	...
	if($wid) {
		...
		$wmset = apms_widget_config($wid, $opt, $mopt, $addon, 'Y');
		$setup_href = '';
...
function apms_board_rows($arr) {
	...
	switch($sort) {
		...
		default				: $orderby1 = 'bn_id desc'; $orderby2 = 'wr_datetime desc'; break;
	}
	...
	if(!$bo_table || count($board_cnt) > 1 || $except) { //복수
		...
		# UNION으로 함께 돌려버린다. 
		# bn = board_number(length)
		$sql_common = " FROM ( ";
		for($bn=0; $bn < count($board_cnt); $bn++){

			$sql_common .= $bn != 0 ? " UNION ALL " : "";

			$sql_common .= "
				SELECT '" . trim($board_cnt[$bn]) . "' as bo_table, wr_datetime, wr_id FROM g5_write_".trim($board_cnt[$bn])." WHERE $sql_mode2 $sql_mode3 $sql_term $sql_main $sql_image $sql_mb $sql_where ";
		}
		$sql_common .= " ) as tem ";
		# Union End 
		...
		#echo " SELECT *  $sql_common ORDER BY $sql_orderby $orderby2 LIMIT $start_rows, $rows ";
