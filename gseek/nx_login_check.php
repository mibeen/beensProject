...
$mb_id       = trim($_SESSION['I_CODE']);
$mb_password = trim($_SESSION['I_CODE']);
...
if (!$mb['mb_id'] || ($mb['mb_level'] != '2' && (($PW_TYPE == 'A' && !check_password($mb_password, $mb['mb_password'])) || ($PW_TYPE == 'B' && F_xenc($mb_password) != $mb['mb_password'])))) {
...
if($_REQUEST["url"])
	$link = explode('?url=', $_SERVER['REQUEST_URI'])[1];
//echo $link;
goto_url($link);
?>
