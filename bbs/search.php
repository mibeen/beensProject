...
if(!$sfl) $sfl = 'wr_1||wr_2||wr_3||wr_subject||wr_content';
...
if ($stx) {
    ...
    for ($idx=$table_index; $idx<count($search_table); $idx++) {
        ...
        for ($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            # - 사업관리 : 모집안내 게시판이면 (달력 게시판이고, IDX값이 있으면.)
            if ($search_table[$idx] == 'calendar' && strlen($row['wr_4']) > 0)
            {
                $list[$idx][$i]['href']     = '/evt/evt.read.php?&ord=1d&EM_IDX=' . $row['wr_4'];
                $list[$idx][$i]['nx_event'] = 'true';
            }
            # - 사업관리 : 종료.
...
if( is_file($skin_path) ){
    ...
    include_once($skin_path.'/basic/search.skin.php');
}
