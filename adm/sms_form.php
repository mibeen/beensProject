...
if ($w == 'u') {
    ...
    $sql = " select * from nx_sms where ma_id = '{$ma_id}' And ma_target = 'A' ";
    $ma = sql_fetch($sql);
    if (!$ma['ma_id'])
        alert('등록된 자료가 없습니다.');
} else {
...
<p>SMS 내용에 {이름} , {닉네임} , {회원아이디} , {휴대폰번호} 처럼 내용에 삽입하면 해당 내용에 맞게 변환하여 SMS를 발송합니다.</p>
...
<input type="hidden" name="ma_target" value="A" id="ma_target">
