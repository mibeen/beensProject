<?php
	...
	class NX_MAIL
	{
		/* smtp 의 호스트 설정 : gmail 은 ssl://smtp.gmail.com */
		var $host="mail.gill.center";
		/* smtp 의 포트 설정 : gmail 은 465 */
		var $port="587";
		/* smtp 계정 아이디 입력 */
		var $smtp_id="gillcenter@gill.center";
		/* smtp 계정 비밀번호 입력 */
		var $smtp_pw="gillcenter1234!@#$";
		/* 보내는 이메일 */
		var $from_email="no-reply@gill.center";
		...
		function NX_MAIL($data=false) {
			if($data!=false) {
				if(is_array($data)){
					...
					$this->from_email = !empty($data['from_email'])?$data['from_email']:$this->from_email;
