...
<div class="form-group">
	<label class="col-sm-2 control-label" for="wr_subject">설명<strong class="sound_only">필수</strong></label>
	<div class="col-sm-10">
		<div class="input-group">
			<input type="text" name="wr_2" value="<?php echo $write['wr_2'] ?>" id="wr_subject" required class="form-control input-sm" size="120" maxlength="255">
		</div>
	</div>
</div>

<div class="form-group">
	<label class="col-sm-2 control-label" for="wr_subject">프로필사진<strong class="sound_only">필수</strong></label>
	<div class="col-sm-10">
		<div class="input-group col-md-12">
			<input type="file" name="nx-profile" id="nx-profile">
			<input type="hidden" name="wr_4" id="wr_4" value="<?php echo $write['wr_4']?>" class="wr_subject form-control input-sm" required placeholder="사진을 업로드하시거나, 프로필 이미지의 링크를 적어주세요.">
			<?php 

				if(isset($write['wr_4']) && strlen($write['wr_4']) > 1){
					$filename = explode('/', $write['wr_4']);
					$filename = array_reverse($filename);
					$filename = $filename['1'].$filename['0'];
					echo "<div class=\"form-group\"><label class=\"checkbox-inline\"> ".$filename."</label> | <a href=\"".$write['wr_4']."\"  target=\"_blank\">열기</a></div>";
				}
			?>
		</div>
	</div>
...
<div class="form-group">
	<label class="col-sm-2 control-label" for="wr_subject">작성자 설명<strong class="sound_only">필수</strong></label>
	<div class="col-sm-10">
		<div class="input-group col-md-12">
			<textarea name="wr_3" id="wr_3" style="width: 100%;"><?php echo $write['wr_3']?></textarea>
		</div>
	</div>
</div>
...
$(function(){
	$(document).on('change', '#nx-profile', function(e){
		nx_img_upload('nx-profile', setUrl);
	})
})
