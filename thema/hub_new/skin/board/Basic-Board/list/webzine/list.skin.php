...
?>
<style>
	.list-wrap .list-container { overflow:hidden; /*margin-right:<?php echo ($gap_right > 0) ? '-'.$gap_right : 0;?>px;*/ margin-bottom:<?php echo ($gap_bottom > 15) ? 0 : 15;?>px; /*padding-left: <?php echo $gap_right;?>px;*/ }
	.list-wrap .list-row { float:left; /*width:<?php echo $item_w;?>%;*/ width: calc( (100% - <?php echo ($cols - 1) * $gap_right ?>px) / <?php echo $cols ?>  ); margin-right: <?php echo $gap_right; ?>px;margin-bottom:<?php echo $gap_bottom;?>px; }
	.list-wrap .list-row:nth-child(<?php echo $cols;?>n){margin-right: 0;}
			...
			<div class="list-item">
				...
				<h2>
					<a href="<?php echo $list[$i]['href'];?>"<?php echo $ellipsis.$is_modal_js.$is_link_target;?>>
						...
						<?php } else { ?>
							<?php echo "<p class=\"nx-list-title\">".$list[$i]['subject']."</p>";?>
						<?php } ?>
						...
						<p class="nx-list-content"><?php echo cut_str($list[$i]['wr_2'], 20);?></p>

						<p class="nx-list-date"><?php echo date("Y.m", $list[$i]['date']); ?></p>
					</a>
				</h2>
				<?php
				/*
