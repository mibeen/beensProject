...
// 2017-12-05 Hoyeongkim | 날짜를 관리자에서 수동으로 입력한다. (데이터 이관 관련 박형주 실장 요청, 오전회의에서 결정.)
$wr_writedate = (isset($_POST['wr_datetime']) && strlen(trim($_POST['wr_datetime'])) > 1 ) ? clean_xss_tags($_POST['wr_datetime']) : G5_TIME_YMDHIS;
...
for ($i=1; $i<=10; $i++) {
    ...
    if( isset($_POST['sh_'.$i]) && is_array($_POST['sh_'.$i])){
        //넘어온 값이 있고, 배열이면

        settype($$var, 'string');
        //변수 선언.

        for($m=0; $m<sizeof($_POST['sh_'.$i]); $m++){
            $$var .= $m == 0 ? $_POST['sh_'.$i][$m] : ",".$_POST['sh_'.$i][$m];
        }

        //$$var = trim($_POST['sh_'.$i]);
        continue;

    }

    if (isset($_POST['sh_'.$i]) && settype($_POST['sh_'.$i], 'string')) {
        $$var = trim($_POST['sh_'.$i]);
    }
}
...
if ($w == '' || $w == 'r') {
    ...
    // 새글 INSERT
    if($is_new) sql_query(" insert into {$g5['board_new_table']} ( bo_table, wr_id, wr_parent, bn_datetime, mb_id, as_reply, as_re_mb ) values ( '{$bo_table}', '{$wr_id}', '{$wr_id}', '{$wr_writedate}', '{$member['mb_id']}', '{$wr_reply}', '{$as_re_mb}' ) ");
    ...
    if(isset($_POST['g5_extend']) && $_POST['g5_extend'] == '1'){


        $sql = "INSERT INTO g5_board_extend set
                bo_table = '{$bo_table}',
                wr_id    = '{$wr_id}',
                wr_1     = '{$sh_1}',
                wr_2     = '{$sh_2}',
    ...
    // New Board Table - writedate update
    // 1. UPDATE할 때 마다 글이 전부 New가 되어 버릴 수도 있다.
    // 2. 하지만 어자피 글의 생성 날짜를 컨트롤하는 거니까 상관 없을 듯 하다.
    $sql = " UPDATE {$g5['board_new_table']} SET
                        bn_datetime = '{$wr_writedate}' 
    ...
    */
    ...
    // 학술정보 변경
    $sql = "UPDATE g5_board_extend set
                wr_1     = '{$sh_1}',
                wr_2     = '{$sh_2}',
                wr_3     = '{$sh_3}',
                wr_4     = '{$sh_4}',
...
for ($i=0; $i<count($_FILES['bf_file']['name']); $i++) {
    ...
    if (isset($_POST['bf_file_del'][$i]) && $_POST['bf_file_del'][$i]) {
        ...
        if ($fp) {
            @fwrite($fp, print_r("write_update.655 : " . date("Y-m-d H:i:s",time()) . " : ".G5_DATA_PATH.'/file/'.$bo_table.'/'.$row['bf_file'], true ) ."\n");
            @fclose($fp);
        }
    ...
    if (is_uploaded_file($tmp_file)) {
        ...
        if ($w == 'u') {
            ...
            $fp = @fopen(G5_DATA_PATH.'/log/unlink_log.txt', 'ab');
            if ($fp) {
                @fwrite($fp, print_r("write_update.723 : " . date("Y-m-d H:i:s",time()) . " : ".G5_DATA_PATH.'/file/'.$bo_table.'/'.$row['bf_file'], true ) ."\n");
                @fclose($fp);
            }
...
if($board['as_resize_kb'] > 0 && $board['as_resize'] > 0) {
    ...
    while ($row = sql_fetch_array($result)) {
		...
		if($thumb && $thumb != $org) {
			...
			$fp = @fopen(G5_DATA_PATH.'/log/unlink_log.txt', 'ab');
			if ($fp) {
			    @fwrite($fp, print_r("write_update.971 : " . date("Y-m-d H:i:s",time()) . " : ".$orgfile, true ) ."\n");
			    @fclose($fp);
			}
			...
			$fp = @fopen(G5_DATA_PATH.'/log/unlink_log.txt', 'ab');
			if ($fp) {
			    @fwrite($fp, print_r("write_update.983 : " . date("Y-m-d H:i:s",time()) . " : ".$thumbfile, true ) ."\n");
			    @fclose($fp);
			}
	...
	for($i=0; $i<count($matches[1]); $i++) {
        ...
        if(is_file($srcfile)) {
			...
			if($thumb && $thumb != $org) {
				...
				$fp = @fopen(G5_DATA_PATH.'/log/unlink_log.txt', 'ab');
				if ($fp) {
				    @fwrite($fp, print_r("write_update.1064 : " . date("Y-m-d H:i:s",time()) . " : ".$orgfile, true ) ."\n");
				    @fclose($fp);
				}
				...
				$fp = @fopen(G5_DATA_PATH.'/log/unlink_log.txt', 'ab');
				if ($fp) {
				    @fwrite($fp, print_r("write_update.1076 : " . date("Y-m-d H:i:s",time()) . " : ".$thumbfile, true ) ."\n");
				    @fclose($fp);
				}
			}
		}
