...
if(!$_POST['cf_cert_use']) {
    ...
    // 메인 로고 업로드
    if (is_uploaded_file($_FILES['mb_mlogo']['tmp_name'])) {
        if (!preg_match("/(\.png)$/i", $_FILES['mb_mlogo']['name'])) {
            alert($_FILES['mb_mlogo']['name'] . '은(는) png 파일이 아닙니다.');
        }

		$footer_dir = "mlogo";

        if (preg_match("/(\.png)$/i", $_FILES['mb_mlogo']['name'])) {
            @mkdir(G5_DATA_PATH.'/member/'.$footer_dir, G5_DIR_PERMISSION);
            @chmod(G5_DATA_PATH.'/member/'.$footer_dir, G5_DIR_PERMISSION);

            $dest_path = G5_DATA_PATH.'/member/'.$footer_dir.'/mb_mlogo.png';

            $result = move_uploaded_file($_FILES['mb_mlogo']['tmp_name'], $dest_path);
    ...
    // 푸터 로고 업로드
    if (is_uploaded_file($_FILES['mb_footerfile']['tmp_name'])) {
        if (!preg_match("/(\.png)$/i", $_FILES['mb_footerfile']['name'])) {
            alert($_FILES['mb_footerfile']['name'] . '은(는) png 파일이 아닙니다.');
        }
        ...
        if (preg_match("/(\.png)$/i", $_FILES['mb_footerfile']['name'])) {
            ...
            $dest_path = G5_DATA_PATH.'/member/'.$footer_dir.'/footer_logo_img.png';

            move_uploaded_file($_FILES['mb_footerfile']['tmp_name'], $dest_path);
            chmod($dest_path, G5_FILE_PERMISSION);
    ...
    // 푸터 로고 업로드
    if (is_uploaded_file($_FILES['mb_footerfile2']['tmp_name'])) {
        if (!preg_match("/(\.png)$/i", $_FILES['mb_footerfile2']['name'])) {
            alert($_FILES['mb_footerfile2']['name'] . '은(는) png 파일이 아닙니다.');
        }
        ...
        if (preg_match("/(\.png)$/i", $_FILES['mb_footerfile2']['name'])) {
            ...
            $dest_path = G5_DATA_PATH.'/member/'.$footer_dir.'/footer_logo_img2.png';

            move_uploaded_file($_FILES['mb_footerfile2']['tmp_name'], $dest_path);
            chmod($dest_path, G5_FILE_PERMISSION);
    ...
    // 푸터 로고 업로드
    if (is_uploaded_file($_FILES['mb_footerfile3']['tmp_name'])) {
        if (!preg_match("/(\.png)$/i", $_FILES['mb_footerfile3']['name'])) {
            alert($_FILES['mb_footerfile3']['name'] . '은(는) png 파일이 아닙니다.');
        }
        ...
        if (preg_match("/(\.png)$/i", $_FILES['mb_footerfile3']['name'])) {
            ...
            $dest_path = G5_DATA_PATH.'/member/'.$footer_dir.'/footer_logo_img3.png';

            move_uploaded_file($_FILES['mb_footerfile3']['tmp_name'], $dest_path);
            chmod($dest_path, G5_FILE_PERMISSION);
    ...
    // 파비콘 업로드
    if (is_uploaded_file($_FILES['mb_fico']['tmp_name'])) {
        if (!preg_match("/(\.ico)$/i", $_FILES['mb_fico']['name'])) {
            alert($_FILES['mb_fico']['name'] . '은(는) ico 파일이 아닙니다.');
        }
        ...
        if (preg_match("/(\.ico)$/i", $_FILES['mb_fico']['name'])) {
            ...
            $dest_path = G5_DATA_PATH.'/member/'.$footer_dir.'/mb_fico.ico';

            move_uploaded_file($_FILES['mb_fico']['tmp_name'], $dest_path);
            chmod($dest_path, G5_FILE_PERMISSION);
