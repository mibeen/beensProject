...
<div class=" tbl_wrap">
    ...
    <tbody>
    <tr>
        <th scope="row">템플릿 사용</th>
        <td>
            <label for="template_N">일반 모드</label><input type="radio" name="template" id="template_N" value="N" <?php echo !isset($nw['template']) ? 'checked' : ''; ?> >
            <label for="template_Y">템플릿 모드</label><input type="radio" name="template" id="template_Y" value="Y" <?php echo isset($nw['template']) ? 'checked' : ''; ?> >

            <div id="template_case" style="display: none">
    ...
    <tr>
        ...
        <td>
            ...
            <input type="radio" name="nw_view_status" id="status_top" value="top" <?php echo $nw['nw_view_status'] == 'top' ? 'checked' : '' ?>><label for="status_top">상단팝업</label>
        </td>
    </tr>    
    <tr class="hidden" id="layer_bg">
    </tr>
    <tr>
...
<script>
$(function(){
    // Popup Layer Custom Function
    $(document).on('change', 'input[name=template]', function(e){
        var checked_value = $('input[name=template]:checked').val();
...
/*
 * function layer_open(bullen)
 * open / close / default = open
 */
function layer_open(bull){

    switch(bull){
        case 'open' :
            $('#template_case').show();
            $('#layer_bg').html("<th scope=\"row\"><label for=\"nw_height\">팝업레이어 배경<strong class=\"sound_only\"> 필수</strong></label></th><td><input type=\"text\" name=\"nw_url\" value=\"<?php echo $nw['nw_url'] ?>\" id=\"nw_url\" required class=\"frm_input required\" size=\"5\"><input type='hidden' name='attachment' id='nw_attachment' value=''></td>");
            break;
        case 'close' :
            $('#template_case').hide();
            $('#layer_bg').html('');
