<?php
	...
	if (!$is_admin) {
		$wh .= " And EP.EP_IDX In(Select EPM.EP_IDX From NX_EVENT_PROJECT_MEMBER As EPM Where EPM.mb_id = '" . mres($member['mb_id']) . "')";
	}
	if ($sc_ep_s_date != '') {
...
if ($is_admin) {
	?>
<div class="taR mb10">
	<a href="project.add.php" class="nx-btn-b2">사업 등록</a>
</div>
	<?php
}
?>
...
<table border="0" cellpadding="0" cellspacing="0" class="nx-t-list1">
	...
	<tbody>
		...
		if (sql_num_rows($row) <= 0) {
			echo '<tr><td colspan="'.(($is_admin) ? '4': '3').'">등록된 공모사업이 없습니다.</td></tr>';
		}
		else {
			$s = 0;
		...
		<tr>
			...
			if ($is_admin) {
				?>
			<td>
				<a href="project.edit.php?<?php echo($phpTail)?>EP_IDX=<?php echo($rs1['EP_IDX'])?>" class="nx-btn2">수정</a>
			</td>
				<?php
			}
			?>
