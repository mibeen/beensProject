...
<style>
	...
	.nx_btn_cmt_add{min-width:84px; height:30px; line-height:30px; color:#fff; border-radius:4px; transition:background 0.25s ease-in-out; background:#3f9de3;}
</style>
...
<?php if($member['mb_id']){ ?>
            <div class="nx_btn_area">
                <div class="ofH">
					...
					*/ ?>
					<a onclick="winLearn('<?php echo $result_ary[button_study_link];?>', '<?php echo $url; ?>', '<?php echo str_replace("'", "\\'", htmlspecialchars($result_ary[buttion_study_name]));?>');" class="nx_btn type1"><?php echo $result_ary[buttion_study_name];?></a>
                    <?php }
                    // 학습기간 이전일 경우
