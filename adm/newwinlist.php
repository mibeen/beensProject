...
if(!sql_query(" DESCRIBE {$g5['new_win_table']} ", false)) {
    if(sql_query(" DESCRIBE {$g5['g5_shop_new_win_table']} ", false)) {
       ...
       $query_cp = sql_query(" CREATE TABLE IF NOT EXISTS `{$g5['new_win_table']}` (
                                   ...
                                   `nw_background` varchar(255) NOT NULL DEFAULT '',
                                   `nw_attachment` varchar(3) NOT NULL DEFAULT 'A',
                                   `nw_view_status` enum('layer','window','top') NOT NULL DEFAULT 'layer',
                                   `template` varchar(10) NOT NULL DEFAULT '',
                                   `nw_padding` varchar(10) NOT NULL DEFAULT '15',
                                   `nx_href` varchar(255) NOT NULL DEFAULT '',
                                   `nx_target` varchar(100) NOT NULL DEFAULT '_self',
                                   PRIMARY KEY (`nw_id`,`nw_content_html`)
                    ) ENGINE=MyISAM DEFAULT CHARSET=utf8 ", true);
    }
}


#Template Modify Query
$makeSql = "ALTER TABLE g5_new_win
    ADD COLUMN nw_background VARCHAR(255) NOT NULL DEFAULT '' AFTER nw_content_html,
    ADD COLUMN nw_attachment ENUM('A','B','C') NOT NULL DEFAULT 'A' AFTER nw_background,
    ADD COLUMN nw_view_status ENUM('layer','window','top') NOT NULL DEFAULT 'layer' AFTER nw_attachment,
    ADD COLUMN template VARCHAR(10) NOT NULL DEFAULT '' AFTER nw_view_status";
sql_query($makeSql);
