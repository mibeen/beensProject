...
//nx_img_upload
// nx_img_upload($id-name, $form-name)
function nx_img_upload($idname, callback){

    if(!$idname){
        console.log("Error : nx_img_upload()의 인수가 정의되지 않았습니다.");
        return false;
    }

        if($('#' + $idname).val().length < 1){
            //이미지 없을 때
...
//login_chk
function nx_login_chk(){

    var result = '';

    $.ajax({
        'url'     : '/bbs/ajax.loginchk.php',
        'method'  : 'POST',
        'success' : function(data){
...
$(function(){
    ...
    /*$(document).on('click', '.nx-auth', function(event){
        event.preventDefault();
        var login = nx_login_chk();
        console.log(login);
        if( login == true){
