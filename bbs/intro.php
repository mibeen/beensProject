...
list($intro_type, $intro_stime, $intro_etime) = explode("|", $intro_set);

if($intro_type == "2") { // 지정날짜
	if((int)$intro_stime < G5_SERVER_TIME && (int)$intro_etime > G5_SERVER_TIME) {
		if(!$is_admin)
			$is_intro = true;
	} else if ((int)$intro_stime < G5_SERVER_TIME && (int)$intro_etime < G5_SERVER_TIME) {
...
if($is_intro) {
	if(is_file(G5_SKIN_PATH.'/intro/'.$intro_skin.'/intro.html')) {
		...
		$start_datetime = (int)$intro_stime;
		$end_datetime = (int)$intro_etime;
		$skin = $intro_skin; 
