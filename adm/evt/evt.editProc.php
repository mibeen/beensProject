<?php
	...
	$EM_ADDR_USE_YN = $_POST['EM_ADDR_USE_YN'];
	...
	$EM_ADDR1 = $_POST['EM_ADDR1'];
	$EM_ADDR2 = $_POST['EM_ADDR2'];
	...
	$EM_ADDR_USE_YN = F_YN($EM_ADDR_USE_YN,'N');
	...
	# get data
	$db1 = sql_fetch("Select EM_IDX, EM_S_DATE, EM_JOIN_PREFIX From NX_EVENT_MASTER As EM Where EM.EM_DDATE is null And EM.EM_IDX = '" . mres($EM_IDX) . "' Limit 1");
	...
	if ($DB_EM['EM_S_DATE'] != $EM_S_DATE)
	{
		...
		# get : 접수된 인원 count
		$row = sql_fetch("Select Count(EJ_IDX) As cnt From NX_EVENT_JOIN Where EJ_DDATE is null And EM_IDX = '" . mres($EM_IDX) . "'");
		if ($row['cnt'] > 0) {
			echo_json(array(
				'success'=>(boolean)false, 
	...
	$sql = "Update NX_EVENT_MASTER Set"
		...
		.", EM_ADDR1 = '" . mres($EM_ADDR1) . "'"
		.", EM_ADDR2 = '" . mres($EM_ADDR2) . "'"
		.", EM_JOIN_PREFIX = '" . mres($EM_JOIN_PREFIX) . "'"
	    .", EM_ADDR_USE_YN = '" . mres($EM_ADDR_USE_YN) . "'"
