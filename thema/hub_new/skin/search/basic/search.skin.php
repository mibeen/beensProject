...
add_stylesheet('<link rel="stylesheet" href="'.$skin_url.'/basic/style.css" media="screen">', 0);

#for 사업관리

 # OPTION 그룹에 사업관리를 넣는다.
 $group_option .= "<option value=\"NX_EVENT_MASTER\">모집정보</option>";

?>
<script>
console.log(<?php echo($header_skin)?>);
</script>
<style>
.div-title{font-size:12pt;}
.at-main.nx_page_content{border-right: none; width: 100%;}
...
?>
	...
	for ($i=0; $i<count($list[$idx]) && $k<$rows; $i++, $k++) {
		...
		# - 사업관리 쪽 데이터이면
		$nx_bo_table = $list[$idx][$i]['bo_table'];
		$nx_bo_idx   = $list[$idx][$i];
		if( $list[$idx][$i]['nx_event'] == 'true' )
		{

			$sql = "Select EM.EM_IDX, EM.EM_TITLE, EM.EM_CONT, EM.EM_S_DATE, EM.EM_E_DATE, EM.EM_S_TIME, EM.EM_E_TIME"
				...
				."	Where EM.EM_DDATE is null AND EM.EM_IDX =" . $list[$idx][$i]['wr_4']
				."	Order By EM.EM_IDX Desc "
				."	Limit 0, 1 "
				;
			$rs1 = sql_fetch($sql, false);
			$thumb      = thumbnail($rs1['bf_file'], G5_PATH."/data/file/NX_EVENT_MASTER", G5_PATH."/data/file/NX_EVENT_MASTER", 64, 64, true);
			$img['src'] = '/data/file/NX_EVENT_MASTER/' . $thumb; 
			...
			$img = apms_wr_thumbnail($nx_bo_table, $nx_bo_idx, 80, 80, false, true); // 썸네일	
	 ...
	 ?>
		<div class="media">
			...
			<div class="media-body">
				<a href="<?php echo $list[$idx][$i]['href'] ?><?php echo $comment_href ?>" target="_blank" class="pull-right"><span class="text-muted font-12">새창</span></a>
				...
				<div class="media-info font-12 text-muted">
				...
				<div class="media-content">
					<a href="<?php echo $list[$idx][$i]['href'] ?><?php echo $comment_href ?>">
						<?php if( $list[$idx][$i]['nx_event'] == 'true' ){
							echo "<span class=\"text-muted\">" . apms_cut_text($rs1['EM_CONT'], 60, '… <span class=\"font-11 text-muted\">더보기</span>') . "</span>";
						}else{
							echo "<span class=\"text-muted\">" . apms_cut_text($list[$idx][$i]['content'], 60, '… <span class=\"font-11 text-muted\">더보기</span>') . "</span>";
						} ?>
					</a>
				</div>
			</div>
