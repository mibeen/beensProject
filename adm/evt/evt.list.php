<?php
	...
	if ($EP_IDX != "") {
		$sql = "Select EP_TITLE From NX_EVENT_PROJECT Where EP_DDATE is null And EP_IDX = '" . mres($EP_IDX) . "'";
		$db_ep = sql_fetch($sql);
	...
	$g5[title] = ($sub_menu == '980100') ? "공모사업 행사목록" : "사업관리";
...
<h3 class="nx-tit1 lh30 mb" style="margin-top:0px">
	<a href="project.list.php" class="nx-btn-b3 fR ml15">공모사업 목록</a>
...
<div class="taR mb10">
	...
	?>
	<a href="<?php echo($add_link)?>" class="nx-btn-b2">행사 만들기</a>
</div>
...
<table border="0" cellpadding="0" cellspacing="0" class="nx-t-list1">
	...
	<tbody>
		...
		if ($total_count <= 0 || sql_num_rows($row) <= 0) {
			echo '<tr><td colspan="7">등록된 행사가 없습니다.</td></tr>';
		}
		else {
