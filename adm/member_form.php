...
else if ($w == 'u')
{
    ...
    # 관리자 개인정보 접근이력 기록
    nx_privacy_log('read', $g5['member_table'], $mb['mb_id']);
...
$g5['title'] .= $mmode_title[$mmode] . ' ' .$html_title;
    ...
    <tr>
        ...
        <td>
            ...
            <select id="mb_level" name="mb_level">
                <option value="<?php echo($mmode_level[$mmode])?>"<?php if($mb['mb_level'] == $mmode_level[$mmode]){echo(' selected');}?>><?php echo($mmode_level[$mmode])?></option>
            </select>
                <?php
            }
            ?>
        </td>
        <th scope="row" style="display:none;">포인트</th>
        <td style="display:none;"><a href="./point_list.php?sfl=mb_id&amp;stx=<?php echo $mb['mb_id'] ?>" target="_blank" ><?php echo number_format($mb['mb_point']) ?></a> 점</td>
    </tr>
	<?php if($mb['as_date']) { ?>
		<tr>
...
<div class="btn_confirm01 btn_confirm">
    ...
    <a href="./member_list.php?<?php echo $qstr ?>&amp;mode=<?php echo $mmode ?>">목록</a>
</div>
</form>
