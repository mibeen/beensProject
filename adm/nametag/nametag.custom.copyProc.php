<?php
	...
	$ENC_TITLE = $_POST['ENC_TITLE'];
	$ENT_IDX = $_POST['ENT_IDX'];


	# re-define
	$ENC_IDX = CHK_NUMBER($ENC_IDX);
	$ENC_TITLE = trim($ENC_TITLE);
	$ENT_IDX = CHK_NUMBER($ENT_IDX);


	#----- chk : rfv.
	$rfv = array();
	if ($ENT_IDX < 0 || $ENC_TITLE == '') $rfv[] = ['str'=>'잘못된 접근 입니다.'];
	...
	# insert : NX_EVENT_NAMETAG_CUSTOM
	# ENC_IDX 가 없을 경우 생성
	if ($ENC_IDX == '') {
		$sql = "Insert Into NX_EVENT_NAMETAG_CUSTOM("
		."ENC_TITLE"
		.", mb_id"
		.", ENC_WDATE"
		.", ENC_WIP"
		...
		# set : ENC_IDX
		$ENC_IDX = sql_insert_id();
	}


	#-----
	# ENC_IDX 가 넘어왔을 경우 기존의 정보가 있는지 확인하여 삭제 처리
	if ($ENC_IDX > 0)
	{
		$sql = "Select ENT_IDX From NX_EVENT_NAMETAG_TEMPLATE Where ENT_DDATE is null And ENT_IDX != '" . mres($ENT_IDX) . "' And ENC_IDX = '" . mres($ENC_IDX) . "' Order By ENT_IDX Asc";
		$db1 = sql_query($sql);
	...
	$sql = "Insert Into NX_EVENT_NAMETAG_TEMPLATE("
		...
		.", ENT_WIP"
		.")"
		." Select " . (($ENC_IDX > 0) ? "'".mres($ENC_IDX)."'" : 'null')
		.", ENT_TITLE"
		.", ENT_WIDTH"
		.", ENT_HEIGHT"
		.", ENT_F1_KIND"
	...
	echo_json(array(
		...
		'redir'=>'nametag.custom.edit.php?ENC_IDX='.$ENC_IDX,
		'msg'=>(($ENC_IDX > 0) ? "" : "복제되었습니다.")
	));
?>
