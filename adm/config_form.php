...
// Congig - for footer addr
sql_query(" ALTER TABLE `{$g5['config_table']}` CHANGE `cf_1` `cf_1` TEXT NOT NULL DEFAULT '' ", false);
...
$pg_anchor = '<ul class="anchor">
    ...
    <li><a href="#anc_cf_etc">사이트 기타설정</a></li>
...
<section id="anc_cf_basic">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        if ($is_admin == 'super') {
            ?>
        <tr>
            <th scope="row"><label for="cf_10">관리자 접근가능 IP</label></th>
            <td colspan="3">
                <?php echo help('입력된 IP에서만 관리자 페이지에 접근할 수 있습니다.<br>123.123.+ 도 입력 가능. (엔터로 구분)<br>61.36.81.137은 항상 접근 가능합니다.') ?>
                <textarea name="cf_10" id="cf_10"><?php echo $config['cf_10'] ?></textarea>
            </td>
        </tr>
            <?php
        }
...
<section id="anc_cf_join">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <th scope="row"><label for="cf_register_point" style="display:none;">회원가입시 포인트</label></th>
            <td><input type="text" name="cf_register_point" value="<?php echo $config['cf_register_point'] ?>" id="cf_register_point" class="frm_input" size="5" style="display:none;"> 점</td>
        </tr>
        <tr>
...
<section id="anc_cf_extra">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tbody>
        <tr>
            <th colspan="2" scope="row" style="font-size:15pt; text-align:left;" >
            <a href="javascript:fcio_up();"  class="btn_frmline" style="padding:0px 10px 10px 10px; text-align:center;">파비콘 관리</a>&nbsp;
            <a href="javascript:fcio_del();"  class="btn_frmline" style="padding:0px 10px 10px 10px; text-align:center; background: #38b7ea;">파비콘 전체 삭제</a>
            <script>
            function fcio_up()
            {
                window.open("config_form_multi.php", "_blank", "toolbar=no,scrollbars=yes,resizable=yes,top=100,left=500, width=1000, height=500"); 
            }

            function fcio_del()
            {
                if(confirm("파비콘을 삭제하시겠습니까?")){
                    window.open("config_form_multi_delete.php", "_blank", "toolbar=no,scrollbars=no,resizable=no,top=100,left=100, width=500, height=300"); 
                }
            }
            </script>
            </th>
        ...
        <tr>
            ...
            {
                if(in_array(pathinfo($files1[$i], PATHINFO_EXTENSION), $valid_formats) ){
                echo "<a href='". G5_DATA_URL .'/member/ico/'. utf8_decode( $files1[$i] )."' target='_blank'>". utf8_decode( $files1[$i] ) ."</a><br/>";
                }
            }
            ?>
            </td>
        ...
        <tr style="display:none;">
            <td colspan="2">
                <input type="file" name="mb_fico" id="mb_fico">
                <?php

                $footer_dir = "fico";
                $footer_file = G5_DATA_PATH.'/member/'.$footer_dir.'/mb_fico.ico';
                if (file_exists($footer_file)) {
                    $footer_url = G5_DATA_URL.'/member/'.$footer_dir.'/mb_fico.ico';
        ...
        <tr>
            <th colspan="2" scope="row" style="font-size:15pt; text-align:left;">로고관리</th>
        </tr>
        <tr>
            <td colspan="2">
                <input type="file" name="mb_mlogo" id="mb_mlogo">
                <?php
                $footer_dir = "mlogo";
        ...
        <?php for ($i=1; $i<=10; $i++) { 
            // 2번 - m3stats.php 파일에서 사용
            // 3번 - m3stats.php 파일에서 사용
            // 8번 - 어드민스킨용
            // 10번 - 관리자 페이지 접근 허용 IP
            ?>
            ...
            if ($i == 1) {
                ?>
            <tr>
                <th colspan="2" scope="row" style="font-size:15pt; text-align:left;">푸터관리</th>
            </tr>
            <tr>
                <td colspan="2">
                    <p class="bold text-left">footer left</p>
                    <input type="file" name="mb_footerfile" id="mb_footerfile">
                    <?php
                    $footer_dir = "footer";
                    ...
                    <p class="bold text-left">footer right 1</p>
                    <input type="file" name="mb_footerfile2" id="mb_footerfile2">
                    <?php
                    $footer_dir = "footer";
                    $footer_file = G5_DATA_PATH.'/member/'.$footer_dir.'/footer_logo_img2.png';
                    if (file_exists($footer_file)) {
                        $footer_url = G5_DATA_URL.'/member/'.$footer_dir.'/footer_logo_img2.png';
                        echo '<img src="'.$footer_url.'" alt="">';
                    }
                    ?>
                    ...
                    <p class="bold text-left">footer right 2</p>
                    <input type="file" name="mb_footerfile3" id="mb_footerfile3">
                    <?php
                    $footer_dir = "footer";
                    $footer_file = G5_DATA_PATH.'/member/'.$footer_dir.'/footer_logo_img3.png';
                    if (file_exists($footer_file)) {
                        $footer_url = G5_DATA_URL.'/member/'.$footer_dir.'/footer_logo_img3.png';
                        echo '<img src="'.$footer_url.'" alt="">';
                    }
                    ?>
                </td>
            ...
            <tr>
                <td colspan="2" class="td_extra">
                    <input type="text" name="cf_<?php echo ($i == 8) ? $i.'_old' : $i; ?>_subj" value="<?php echo get_text($config['cf_'.$i.'_subj']) ?>" id="cf_<?php echo $i ?>_subj" class="frm_input" size="30" style="display:none;">
                    
                    <textarea name="cf_<?php echo ($i == 8) ? $i.'_old' : $i; ?>" id="cf_<?php echo $i ?>" class="frm_input"  ><?php echo $config['cf_'.$i]; ?></textarea>
                </td>
            </tr>
                <?php 
            } 
...
function fconfigform_submit(f)
{
	...
	if (f.mb_mlogo.value && !f.mb_mlogo.value.match(/\.png/i)) {
        alert('로고는 png 파일만 가능합니다.');
        return false;
    }

	if (f.mb_footerfile.value && !f.mb_footerfile.value.match(/\.png/i)) {
        alert('아이콘은 png 파일만 가능합니다.');
        return false;
    }

	if (f.mb_fico.value && !f.mb_fico.value.match(/\.ico/i)) {
        alert('아이콘은 ico 파일만 가능합니다.');
        return false;
    }
