					...
					<?php if($do_cnt > 0) { ?>
						<div class="media-body">
							...
							<?php for($i = 0; $i < $do_cnt; $i++) { 
								...
								if($list[$cday][$i]['wr_4'] > 0){
									$list[$cday][$i]['href'] = "/evt/evt.read.php?&ord=1d&EM_IDX=" 
															 . $list[$cday][$i]['wr_4'];
							...
							?>
								<li>
									<a href="<?php echo $list[$cday][$i]['href'];?>"<?php echo $list[$cday][$i]['target'];?><?php echo $is_modal_js;?>>
										<p<?php echo ($list[$cday][$i]['wr_3']) ? ' class="'.$list[$cday][$i]['wr_3'].'"' : '';?>>
											<?php echo apms_fa($list[$cday][$i]['as_icon']);?>
											<?php
											if($bgc != ''){
												echo "<span class=\"{$bgc}\">".$list[$cday][$i]['subject']."</span>";
											}else{
												echo $list[$cday][$i]['subject'];
											}
											?>
