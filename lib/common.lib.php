...
// alert + script control (added)
function alert_script($val, $linkVal, $charset="UTF-8")
{
	$ret = "";
	if ( $val != "" || $linkVal != "" )
	{
	    $ret = "<meta http-equiv=\"content-type\" content=\"text/html; charset={$charset}\" />" . chr(13) . chr(10)
	        . "<script type=\"text/javascript\">" . chr(13) . chr(10);

	    if ($val != "") { $ret .= "alert(\"{$val}\");" . chr(13) . chr(10); }
	    if ($linkVal != "") { $ret .= $linkVal . chr(13) . chr(10); }
...
function get_group_select($name, $selected='', $event='')
{
	...
	if ($is_admin == "group") {
		...
		$sql .= " left join $g5[member_table] b on (a.gr_admin REGEXP b.mb_id)
				  where b.mb_id = '$member[mb_id]' ";
	}
...
function member_delete($mb_id)
{
	...
	// 우리동네 학습공간 시설관리자를 공백으로
	sql_query(" update local_place set mb_id = '' where mb_id = '$mb_id' ");
