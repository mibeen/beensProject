<?php
    ...
    */
    ...
    # get : 관리자 메뉴명
    function get_menu_name($menu_num) {
        global $menu;

        $_menu = substr($menu_num, 0, 3);

        $target_menu = $menu['menu'.$_menu];

        $menu_len = Count($target_menu);

        for ($i = 0; $i < $menu_len; $i++) {
            if ($target_menu[$i][0] == $menu_num) {
                return $target_menu[$i][1];
            }
        }
