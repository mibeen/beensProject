...
if(!$wset['lg']) $wset['lg'] = 4;
if(!$wset['md']) $wset['md'] = 3;
if(!$wset['sm']) $wset['sm'] = 2;
if(!$wset['xs']) $wset['xs'] = 1;
...
<div class="tbl_head01 tbl_wrap">
	...
	<tr>
		...
		<td>
			...
			&nbsp;
			라운드
			<input type="text" name="wset[round]" value="<?php echo $wset['round']; ?>" class="frm_input" size="3"> px (1 입력시 원형)
		</td>
	</tr>
	<tr>
		<td align="center" rowspan="2">슬라이드</td>
		<td>
			<label><input type="checkbox" name="wset[auto]" value="1"<?php echo get_checked('1', $wset['auto']); ?>> 자동실행</label>
			&nbsp;
			<label><input type="checkbox" name="wset[rdm]" value="1"<?php echo get_checked('1', $wset['rdm']); ?>> 랜덤섞기</label>
			&nbsp;
			<label><input type="checkbox" name="wset[loop]" value="1"<?php echo get_checked('1', $wset['loop']); ?>> 반복(loop)</label>
			&nbsp;
	...
	<tr>
		<td>
			<input type="text" name="wset[margin]" value="<?php echo $wset['margin']; ?>" class="frm_input" size="4"> px 마진(간격)
			&nbsp;
			<select name="wset[nav]">
				<option value=""<?php echo get_selected('', $wset['nav']); ?>>네비없음</option>
				<option value="1"<?php echo get_selected('1', $wset['nav']); ?>>상단네비</option>
				<option value="2"<?php echo get_selected('2', $wset['nav']); ?>>중앙네비</option>
				<option value="3"<?php echo get_selected('3', $wset['nav']); ?>>하단네비</option>
			</select>
			&nbsp;
			페이저
			...
			&nbsp;
			<label><input type="checkbox" name="wset[in]" value="1"<?php echo get_checked('1', $wset['in']); ?>> 내부 페이저</label>
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>
			<?php echo help('한 개의 슬라이더 내에서 세로배치 갯수');?>
	...
	<tr>
		...
		<td>
			...
			&nbsp;
			<label><input type="checkbox" name="wset[linef]" value="1"<?php echo get_checked('1', $wset['linef']); ?>> 유동 라인</label>
		</td>
	</tr>
	<tr>
