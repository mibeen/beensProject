		...
		<form name="fboardlist" id="fboardlist" action="./board_list_update.php" onsubmit="return fboardlist_submit(this);" method="post" role="form" class="form">
			...
			<?php 
				...
				for ($i=0; $i < $list_cnt; $i++) { 
					...
					#PDF View
					$pdf_view = "";
					$view = get_file($bo_table,$list[$i]['wr_id']);
					for($m=0; $m < count($view['count']); $m++){
						
						/* PDF 문서 바로 보기*/
						if(strpos($view[$m]['source'], ".pdf") !== false) { 

							$pdf_view .= '<a href="'.G5_URL.'/web_pdf/viewer.html?file='. str_replace(G5_URL, "", $view[$m]['path']).'/'.$view[$m]['file'].'" target="_blank" class="list-group-item break-word text-center view_file_download at-tip" style="font-weight:600;">';
							$pdf_view .= '<i class="fa fa-book"></i> PDF 문서 바로 보기</a>';

							$list[$i]['href'] = G5_URL.'/web_pdf/viewer.html?file='. str_replace(G5_URL, "", $view[$m]['path']).'/'.$view[$m]['file'] . '" target="_blanek" ';
			...
			<div class="list-btn-box">
				<?php if ($list_href || $write_href) { ?>
					<div class="form-group pull-right list-btn">
						<div class="btn-group dropup">
							<?php if ($boset['sort']) { ?>
								<a id="sortLabel" data-target="#" href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="btn btn-white btn-sm">
							...
							<?php } ?>
							<?php if ($list_href) { ?><a href="<?php echo $list_href ?>" class="btn btn-white btn-sm"><i class="fa fa-bars"></i><span>목록</span></a><?php } ?>
				...
				<div class="form-group list-btn font-12">
					<div class="btn-group">
						<a href="#" class="btn btn-white btn-sm" data-toggle="modal" data-target="#searchModal" onclick="return false;"><i class="fa fa-search"></i></a>
						<?php if ($rss_href) { ?>
							<a href="<?php echo $rss_href; ?>" class="btn btn-color btn-sm"><i class="fa fa-rss"></i></a>
						<?php } ?>
						<?php if ($is_checkbox || $setup_href || $admin_href) { ?>
							<?php if ($is_checkbox) { ?>
								<button type="submit" name="btn_submit" value="선택삭제" onclick="document.pressed=this.value" class="btn btn-white btn-sm"><i class="fa fa-times"></i><span class="hidden-xs"> 선택삭제</span></button>
								<button type="submit" name="btn_submit" value="선택복사" onclick="document.pressed=this.value" class="btn btn-white btn-sm"><i class="fa fa-clipboard"></i><span class="hidden-xs"> 선택복사</span></button>
								<button type="submit" name="btn_submit" value="선택이동" onclick="document.pressed=this.value" class="btn btn-white btn-sm"><i class="fa fa-arrows"></i><span class="hidden-xs"> 선택이동</span></button>
								<button type="button" id="btn_chkall" class="btn btn-white btn-sm"><i class="fa fa-check"></i><span class="hidden-xs"> 전체선택</span></button>
							<?php } ?>
							<?php if ($admin_href) { ?>
								<a href="<?php echo $admin_href; ?>" class="btn btn-white btn-sm"><i class="fa fa-cog"></i></a>
							<?php } ?>
							<?php if ($setup_href) { ?>
